...
if (!function_exists('wpspace_notice__error')) {
	...
	{
		$niso_info_hide = get_option('niso_info_hide2');
		if (!empty($niso_info_hide)) {
			$niso_info_hide = round((time() - strtotime($niso_info_hide)) / 24 / 60 / 60);
			if ($niso_info_hide < 25) {
				return;
			}
		}

		$class = 'niso-notice notice notice-success is-dismissible';
		...
		printf('<div class="%1$s" style="padding:20px;text-transform:uppercase"><p style="margin-bottom:20px;">%2$s</p><a target="_blank" class="button button-primary" href="%3$s" style="margin-right:10px">' . __('Upgrade Pro', 'niso-carousel-slider') . '</a><a target="_blank" class="button " href="%4$s">' . __('Show Demo', 'niso-carousel') . '</a><a style="margin-left:10px;cursor: pointer;" class="niso-dismiss">' . __('Dissimiss the notice', 'niso-carousel-slider') . '</a></div>', esc_attr($class), wp_kses_post($message), $url1, $url2);
	}
...
if (!function_exists('wpspace_admin_notice_option')) {
	function wpspace_admin_notice_option()
	{
		if (isset($_GET['nhede']) && $_GET['nhede'] == 1) {
			update_option('niso_info_hide2', current_time('mysql'));
		}
	}
...
if (!function_exists('spacehide_go_me')) :
	function spacehide_go_me()
	{
		...
		$url1 = esc_url('https://wpthemespace.com');

		$message = __('<strong><span style="color:red;">Recommended WordPress Theme For You:</span>  <span style="color:green"> If you find a Secure, SEO friendly, full functional premium WordPress theme for your site then </span>  </strong>', 'niso');
